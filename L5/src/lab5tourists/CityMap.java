...
public class CityMap extends Application {
    ...
    private void addEntities() {
        ...
        Label msoe = new Label();
        msoe.setText("Challenge: Find all letters in MSOE\nGoal: MSOE\nFound: ****");
        addChallengeNode(msoe);
        MsoeChallengeObserver msoeChallengeObserver = new MsoeChallengeObserver(msoe);
        for(Car car : cars){
            car.attach(msoeChallengeObserver);
        }

        Label museumLabel = new Label();
        museumLabel.setText("\nChallenge: Find art\nArtistic Works Found: ");

        ImageView museumImage = new ImageView(new Image(CityMap.class.getResource("img/wood-gatherer.png")
                .toString()));
        museumImage.setPreserveRatio(true);
        museumImage.setFitWidth(50);
        museumImage.setVisible(false);
        addChallengeNode(museumLabel);
        addChallengeNode(museumImage);
        MuseumChallengeObserver museumChallengeObserver = new MuseumChallengeObserver(museumLabel, museumImage);

        for(Museum museum: museums){
            museum.attach(museumChallengeObserver);
        }

        Label busLabel = new Label();
        busLabel.setText("\nChallenge: Find all letters in BUS\nGoal: BUS\nFound: ***");
        busLabel.setVisible(false);
        addChallengeNode(busLabel);
        BusChallengeObserver busChallengeObserver = new BusChallengeObserver(busLabel);
        for(Car car :cars){
            car.attach(busChallengeObserver);
        }
        for(Bus bus : buses){
            bus.attach(busChallengeObserver);
        }
    }
}